                  ...
                  {items.map((item, idx) => (
                    <div key={item.id} className="rounded-xl border border-zinc-200 p-3 flex flex-col gap-2 dark:border-zinc-700">
                      ...
                      <div className="grid grid-cols-[1fr_auto] gap-2">
                        <input type="text" placeholder="Descripción del ítem" maxLength={200} value={item.description} onChange={(e) => updateItem(item.id, { description: e.target.value })}
                        ...
                        <input type="number" min="0.01" max="999999999" step="0.01" placeholder="0.00" value={item.amount} onChange={(e) => updateItem(item.id, { amount: e.target.value })}
