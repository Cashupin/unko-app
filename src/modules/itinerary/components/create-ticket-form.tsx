...
export function CreateTicketForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/trips/${tripId}/tickets`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        toast.error(data.error ?? "Error al crear la entrada");
        return;
      }
        ...
        >
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl dark:bg-zinc-900 dark:border dark:border-[#27272a] max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100 dark:border-[#27272a]">
              <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Nueva entrada / ticket</h2>
              <button onClick={() => setOpen(false)} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">✕</button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-5 py-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  autoFocus
                  type="text"
                  placeholder="ej. Parque Ghibli, Pokémon Café…"
                  value={form.title}
                      ...
                      <div className="absolute z-10 mt-1 w-full rounded-lg border border-zinc-200 bg-white shadow-lg dark:border-[#3f3f46] dark:bg-zinc-800 max-h-40 overflow-y-auto">
                        {filteredActivities.slice(0, 8).map((a) => (
                          ...
                          >
                            ...
                            {a.activityDate && (
                              <span className="ml-2 text-xs text-zinc-400">{a.activityDate}</span>
                            )}
                          </button>
                        ))}
                      </div>
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button type="button" onClick={() => setOpen(false)} disabled={loading} className="rounded-lg border border-zinc-200 px-4 py-2 text-sm text-zinc-600 hover:bg-zinc-50 disabled:opacity-50 dark:border-[#3f3f46] dark:text-zinc-400 dark:hover:bg-white/5">
                  Cancelar
                </button>
                <button type="submit" disabled={loading || !form.title.trim()} className="rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-700 disabled:opacity-50 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200">
